...
// SECUR32 functions specific to this BOF
...
void go(char* args, int len)
{
	...
	if (len > 0) {
		targetSPN = (wchar_t*)BeaconDataExtract(&parser, NULL);
	...
	if (!targetDC) {
		...
		if (dwRet == ERROR_SUCCESS && pdcInfo) {
			...
			BeaconPrintf(CALLBACK_ERROR, "[-] Please specify DC manually: exec_bof ldapsecuritycheck.x64.o <SPN> <DC>\n");
			goto cleanup;
		}
	} else {
	...
	if (!targetSPN) {
		...
		BeaconPrintf(CALLBACK_OUTPUT, "[*] Auto-generated SPN: %S\n", finalSPN);
	} else {
		MSVCRT$wcscpy(finalSPN, targetSPN);
	}
