...
)
{
	...
	hr = adcs_com_GetCACertificate(pADCS, ulCertificateServicesServerIndex);
	if (FAILED(hr))
	{
		BeaconPrintf(CALLBACK_ERROR, "adcs_com_GetCACertificate failed: 0x%08lx\n", hr);
		goto fail;
	}


	// Get the WebEnrollmentServers
...
)
{
	...
	// Get the CA Certifiate
	hr = pADCS->pRequest->lpVtbl->GetCACertificate(
		pADCS->pRequest,
		FALSE,
		pADCS->lpCertificateServicesServers[ulCertificateServicesServerIndex].bstrFullName,
		CR_OUT_BINARY,
	...
	if (FAILED(hr))
	{
		BeaconPrintf(CALLBACK_ERROR, "GetCACertificate failed: 0x%08lx\n", hr);
		goto fail;
	}
	else
	{
		internal_printf("bstrCertificate: %S\n", bstrCertificate );
		...
		for(DWORD i=0; i<ulCertificateSize; i++)
		{
		...
		if (FAILED(hr))
		{
			BeaconPrintf(CALLBACK_ERROR, "CertCreateCertificateContext failed: 0x%08lx\n", hr);
			goto fail;
		}
		if(NULL == pCert)
		{
...
)
{
	...
	internal_printf("Certificate Authorities   : %lu CAs\n", pADCS->ulCertificateServicesServerCount);
	internal_printf("================================================================================\n");
	for( ULONG ulCertificateServicesServerIndex=0; ulCertificateServicesServerIndex<pADCS->ulCertificateServicesServerCount; ulCertificateServicesServerIndex++)
	{
		...
		internal_printf("  Web Enrollment Servers  : %lu servers\n", pADCS->lpCertificateServicesServers[ulCertificateServicesServerIndex].ulWebEnrollmentServerCount);
		if ( 
		...
		internal_printf("  Templates               : %lu templates\n", pADCS->lpCertificateServicesServers[ulCertificateServicesServerIndex].ulTemplateCount);
		if ( 
		...
		)
		{
			...
			for( ULONG ulTemplateIndex = 0; ulTemplateIndex<pADCS->lpCertificateServicesServers[ulCertificateServicesServerIndex].ulTemplateCount; ulTemplateIndex++)
			{
				...
				dwFlags = pADCS->lpCertificateServicesServers[ulCertificateServicesServerIndex].lpTemplates[ulTemplateIndex].dwSubjectNameFlags;
				...
				dwFlags = pADCS->lpCertificateServicesServers[ulCertificateServicesServerIndex].lpTemplates[ulTemplateIndex].dwEnrollmentFlags;
